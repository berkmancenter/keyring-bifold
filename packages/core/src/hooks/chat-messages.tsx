  ...
  useEffect(() => {
    ...
    transformedMessages.push(
      ...actionableCredentials.map((record: CredentialExchangeRecord) => {
        ...
        // Detect W3C/JSON-LD credentials by checking the credentialRecordType
        // W3C credentials (VRC, DTG, etc.) have credentialRecordType === 'w3c'
        // AnonCreds credentials don't have this property
        const isJsonLdCredential = record.credentials.some((cred) => cred.credentialRecordType === 'w3c')

        // Get appropriate terminology based on credential type
        // For W3C credentials, query the display registry to get custom terminology
        // For AnonCreds, use default credential terminology
        ...
        if (isJsonLdCredential) {
          // Find the W3C credential record to get its type information
          const w3cCredRecord = record.credentials.find((cred) => cred.credentialRecordType === 'w3c')
          if (w3cCredRecord) {
            const w3cCred = w3cCredentialRecords.find((cred) => cred.id === w3cCredRecord.credentialRecordId)
            if (w3cCred?.credential) {
              // Query the display registry for terminology
              const terminology = credentialDisplayRegistry.getTerminology(w3cCred.credential as any)
        ...
        // Format: "[Name] has sent you their R card" (offer) or "You received a credential from [Name]" (done)
        let eventTitle: string
        let eventSubtitle: string | undefined

        if (record.state === CredentialState.OfferReceived) {
          eventTitle = theirLabel
          eventSubtitle = ' has sent you their R card'
        } else {
          eventTitle = `You received a credential`
          eventSubtitle = ` from ${theirLabel}`
        }

        // Navigation handler for viewing contact details (used inside the bubble for Done state)
        const handleViewDetails = () => {
          if (isJsonLdCredential) {
            ...
            if (w3cCredRecord) {
              const w3cCred = w3cCredentialRecords.find((cred) => cred.id === w3cCredRecord.credentialRecordId)
              if (w3cCred?.credential) {
                const cred = w3cCred.credential as any
        ...
        return {
          _id: record.id,
          text: `${theirLabel} has sent you their R card`,
          renderEvent: () => (
            <View>
              <ChatEvent
                role={Role.them}
