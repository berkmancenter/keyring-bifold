/**
 * Tests for pseudonym utility in core package
 */

import { describe, it, expect } from '@jest/globals'
import { generatePseudonym, truncateDid } from './pseudonym'
...
describe('pseudonym', () => {
  ...
  describe('truncateDid', () => {
    it('truncates long DIDs correctly', () => {
      const did = 'did:peer:2.Ez6LSbty7ZpE4f5tLQVq5aL6GnXfXqTbL4d7f5g5d5x4'
      const truncated = truncateDid(did, 20, 4)
      // With prefixLength=20 and suffixLength=4, result is first 20 chars + "..." + last 4 chars
      expect(truncated).toBe('did:peer:2.Ez6LSbty7...d5x4')
    })

    it('keeps short DIDs unchanged', () => {
      const did = 'did:key:abc'
      const truncated = truncateDid(did)
      expect(truncated).toBe(did)
    })
  })
