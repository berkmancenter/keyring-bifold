...
export class WitnessService {
  ...
  public async verifyPresentation(
    ...
    try {
      ...
      const vrcIssuer = typeof vrcJson.issuer === 'string' ? vrcJson.issuer : vrcJson.issuer?.id
      const vrcSubject = vrcJson.credentialSubject?.id
      console.log(`[${this.name}]   VRC Details:`)
      console.log(`[${this.name}]     Issuer: ${vrcIssuer}`)
      console.log(`[${this.name}]     Subject: ${vrcSubject}`)
      ...
      if (this.config.verbose) {
        ...
        console.log(`[${this.name}]   [CANON-DEBUG] VP without proof:`)
        console.log(`[${this.name}]     ${JSON.stringify(vpWithoutProof, null, 2).split('\n').join('\n      ')}`)
        ...
        console.log(`[${this.name}]   [CANON-DEBUG] VRC without proof:`)
        console.log(`[${this.name}]     ${JSON.stringify(vrcWithoutProof, null, 2).split('\n').join('\n      ')}`)
  ...
  public async issueWitnessCredentials(sessionId: string): Promise<void> {
    ...
    if (!bothWantWitness) {
      ...
      for (const [senderConnectionId, presentation] of presentationEntries) {
        ...
        console.log(`[${this.name}] VWC about ${vrcIssuer}'s VRC → sent to ${recipientConnectionId}`)
